...
import { BarChart3, Globe2, Link2, Building, AlertCircle } from "lucide-react";
import ClientAnalytics from "../dashboard/client/ClientAnalytics";
import { supabase } from "@/config/supabase";

export default function AdminAnalytics() {
  // ===== Site visits (vechiul analytics)
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (cancelled) return;
      if (error) {
        setErrorCompanies(error.message || 'Nu am putut încărca companiile.');
        setCompanies([]);
      } else {
        setCompanies(data || []);
  ...
  return (
    <div className="space-y-8">
      {/* Secțiunea 1: vizite site */}
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-bold text-text-primary flex items-center gap-2">
            <BarChart3 className="w-6 h-6" />
            Analytics trafic (site)
          </h2>
          <select
        ...
        {loadingVisits && <div className="py-8 text-text-secondary">Loading…</div>}
        {errorVisits && (
          <div className="py-8 text-red-600">
            Eroare: {errorVisits.message || String(errorVisits)}
          </div>
        )}
      ...
      {/* Secțiunea 2: Dashboard companii */}
      <div className="space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2 text-text-primary">
            ...
            <div>
              ...
              <h3 className="text-lg font-semibold">Analytics per companie</h3>
            </div>
          </div>
        ...
        {errorCompanies && (
          <div className="flex items-center gap-2 text-red-600 text-sm border border-red-100 bg-red-50 px-3 py-2 rounded-lg">
            ...
            <span>{errorCompanies}</span>
          </div>
        )}

        {!loadingCompanies && !errorCompanies && !selectedCompany && (
