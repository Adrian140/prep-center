...
const isAutoGeneratedShipmentNote = (value) =>
  /shipment\s+created\s+at/i.test(String(normalizeCode(value) || ''));
const resolveClientNote = (payload = {}) => {
  const fields = [
    'client_note',
    'clientNote',
    'customer_note',
  ...
  for (const field of fields) {
    const value = normalizeCode(payload?.[field]);
    if (!value) continue;
