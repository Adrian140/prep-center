...
begin
  ...
  select
    ...
    s.unit_price,
    sum(s.units)::integer,
    coalesce(conf.confirmation_id, pr.fba_shipment_id),
    pr.user_id,
    pr.user_id,
    v_market
  ...
  left join lateral (
    select
      coalesce(
        nullif(elem->>'shipmentConfirmationId', ''),
        nullif(elem->>'shipmentConfirmationID', ''),
        nullif(elem->>'confirmationId', '')
      ) as confirmation_id
    from jsonb_array_elements(coalesce(pr.step2_shipments, '[]'::jsonb)) elem
    where coalesce(
      nullif(elem->>'shipmentConfirmationId', ''),
      nullif(elem->>'shipmentConfirmationID', ''),
      nullif(elem->>'confirmationId', '')
