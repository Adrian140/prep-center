...
return_current as (
  select
    r.company_id,
    sum(coalesce(r.total, r.unit_price * r.units)) as amount
  from public.return_service_lines r
  join companies c on c.company_id = r.company_id
  where r.service_date between p_start_date and p_end_date
    and (
      p_country is null
      or upper(coalesce(r.country, '')) = upper(p_country)
    )
...
return_prev as (
  ...
  from public.return_service_lines r
  join companies c on c.company_id = r.company_id
  where r.service_date between p_prev_start_date and p_prev_end_date
    and (
      p_country is null
      or upper(coalesce(r.country, '')) = upper(p_country)
    )
...
select
  c.company_id,
  coalesce(fc.amount, 0) + coalesce(fmc.amount, 0) + coalesce(oc.amount, 0) + coalesce(rc.amount, 0) as current_sold,
  (coalesce(fp.amount, 0) + coalesce(fmp.amount, 0) + coalesce(op.amount, 0) + coalesce(rp.amount, 0)) - coalesce(ip.amount, 0) as carry,
  (coalesce(fa.amount, 0) + coalesce(fma.amount, 0) + coalesce(oa.amount, 0) + coalesce(ra.amount, 0)) - coalesce(ipa.amount, 0) as live_balance
from companies c
...
left join return_current rc on rc.company_id = c.company_id
...
left join return_prev rp on rp.company_id = c.company_id
...
left join return_all ra on ra.company_id = c.company_id
