...
-- Idempotent policies (drop if exists because create policy lacks IF NOT EXISTS in psql)
drop policy if exists "Public read integration page" on public.integration_page_content;
drop policy if exists "Service role write integration page" on public.integration_page_content;
