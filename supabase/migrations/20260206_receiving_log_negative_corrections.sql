-- Allow negative corrections in receiving_to_stock_log and update stock accordingly
begin;

-- Relax quantity_moved check to allow negative (non-zero) adjustments
alter table public.receiving_to_stock_log
  drop constraint if exists receiving_to_stock_log_quantity_moved_check;

alter table public.receiving_to_stock_log
  add constraint receiving_to_stock_log_quantity_moved_check
  check (quantity_moved <> 0);

-- Update trigger function to apply negative deltas
