...
async function getLwaAccessToken(refreshToken: string, traceId?: string) {
  ...
  console.log("fba-plan-step1-preview lwa-scope", { traceId, scope });
...
async function assumeRole(roleArn: string) {
  ...
  return { accessKeyId, secretAccessKey, sessionToken } as TempCreds;
}
  ...
  try {
    ...
    if (!requestId || !inboundPlanId) {
      return new Response(
        JSON.stringify({
          code: "MISSING_IDS",
          message: "Lipsesc request_id sau inbound_plan_id.",
          traceId
        }),
    ...
    const { data: reqData, error: reqErr } = await supabase
      .from("prep_requests")
      .select("id, destination_country, company_id, user_id, inbound_plan_id")
      .eq("id", requestId)
      .maybeSingle();
    ...
    if (isLockId(inboundPlanId)) {
      return new Response(
        JSON.stringify({
          ...
          message: "Planul Amazon se creeaza. Reincearca in cateva secunde.",
          traceId
        }),
        { status: 202, headers: { ...baseCorsHeaders, "content-type": "application/json" } }
      );
    }
    ...
    const { accessToken: lwaAccessToken } = await getLwaAccessToken(refreshToken, traceId);

    console.log("fba-plan-step1-preview auth-context", {
      traceId,
      inboundPlanId,
    ...
    let listRes = await listPackingOptions();
    const listStatus = listRes?.res?.status ?? null;
    if (listRes?.res?.status === 400 && /empty inbound plan/i.test(listRes?.text || "")) {
      return new Response(
        JSON.stringify({
          code: "PACKING_OPTIONS_NOT_READY",
          message: "Amazon spune ca inbound plan-ul este inca gol. Reincearca dupa cateva secunde.",
          traceId,
          inboundPlanId,
    ...
    if (listRes && !listRes.res.ok) {
      ...
      return new Response(
        JSON.stringify({
          ...
          message: `SP-API listPackingOptions a esuat cu status ${listStatus ?? "n/a"}.`,
          traceId,
          inboundPlanId,
          packingOptionId: null,
          amazonIntegrationId: integId || null,
        ...
        { status, headers: { ...baseCorsHeaders, "content-type": "application/json" } }
      );
    }

    const extractPackingOptionsFromResponse = (res: Awaited<ReturnType<typeof signedFetch>> | null) =>
      res?.json?.payload?.packingOptions || res?.json?.packingOptions || res?.json?.PackingOptions || [];
    const packingOptions = extractPackingOptionsFromResponse(listRes);

    if (!packingOptions.length) {
      warnings.push(
        "Amazon nu a returnat packingOptions pentru acest inbound plan (posibil plan nesuportat pentru packing options)."
      );
      return new Response(
        JSON.stringify({
          code: "PACKING_OPTIONS_NOT_AVAILABLE",
          message: "Amazon nu a returnat packingOptions pentru acest inbound plan. Continuam fara pack groups.",
          traceId,
          inboundPlanId,
          packingOptionId: null,
          amazonIntegrationId: integId || null,
    ...
    const chosen = pickPackingOption(packingOptions, requestedPackingOptionId);
    ...
    if (!packingGroupIds.length) {
      warnings.push("Amazon nu a returnat packingGroupIds pentru packingOptions.");
      return new Response(
        JSON.stringify({
          code: "PACKING_GROUPS_NOT_READY",
          message: "Packing groups nu sunt disponibile inca pentru preview.",
          traceId,
          inboundPlanId,
          packingOptionId,
          amazonIntegrationId: integId || null,
    ...
    console.error("fba-plan-step1-preview error", { traceId, error: e });
